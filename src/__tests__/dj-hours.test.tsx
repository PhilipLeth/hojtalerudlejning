...
import {DJ_DEFAULT, DJ_HOUR_RATE, djHoursFromRange, priceDj} from "@/lib/dj";
...
describe("DJ-timevalg",()=>{
 it("regner start/slut og viser tilbuddet til 1.800 kr/time",()=>{
  render(<Harness/>);
  expect(screen.getByText(`${(3*DJ_HOUR_RATE).toLocaleString("da-DK")} kr inkl. moms`)).toBeInTheDocument();
  ...
  expect(screen.getByText(`${(4*DJ_HOUR_RATE).toLocaleString("da-DK")} kr inkl. moms`)).toBeInTheDocument();
 });
 it("koster det samme tre timer efter kl. 23",()=>{
  render(<Harness/>);
